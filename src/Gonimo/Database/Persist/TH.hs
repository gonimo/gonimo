...
module Gonimo.Database.Persist.TH
    ( -- * Parse entity defs
      persistWith
    , persistUpperCase
    , persistLowerCase
...
-- Added by robert klotzner for our fork:
    ...
    (instDecs, i) <-
      if mpsGeneric mps
        then if not useNewtype
               ...
               then do pfDec <- pfInstD
                       -- Changes by Robert Klotzner: Added ToText and FromText class
                       return (pfDec, [''Show, ''Read, ''Eq, ''Ord, ''Generic, ''ToText, ''FromText])
                else do
                       -- Changes by Robert Klotzner: Added ToText and FromText class
                    let allInstances = [''Show, ''Read, ''Eq, ''Ord, ''PathPiece, ''ToHttpApiData, ''FromHttpApiData, ''PersistField, ''PersistFieldSql, ''ToJSON, ''FromJSON, ''ToText, ''FromText]
                    if customKeyType
