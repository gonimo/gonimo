...
data Authorized
  = Authorized DeviceId FromClient
  | Denied DeviceId ToClient
...
denyView auth@AuthRequest {..} view' =
  case view' of
   ...
   SelectAccountDevice devId -> denyUnless (devId == senderId) -- TODO: Once we have user accounts this check is no longer correct.
