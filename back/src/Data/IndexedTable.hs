...
{-|
Module      : Data.IndexedTable
Description : An indexed table is a map with an additional lookup (the index).
Copyright   : (c) Robert Klotzner, 2017

This module provides indexed tables, where you can lookup values not only by a
key, but also by some property of the value - the index. Think of database
indeces.

If you need multiple indeces, you can nest 'IndexedTable'.
-}
module Data.IndexedTable (
                                          -- * Types
                                          IndexedTable
